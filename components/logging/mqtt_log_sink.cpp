...
bool MQTTLogSink::parseConfig(const std::string& config_str) {
    ...
    if (json) {
        ...
        while (pos != std::string::npos) {
            ...
            else if (key == "broker_port") config_.broker_port = atoi(value.c_str());
            else if (key == "topic") config_.topic = value;
            else if (key == "format") config_.format = value;
            else if (key == "qos") config_.qos = atoi(value.c_str());
    ...
    while (fgets(line, sizeof(line), config_file)) {
        ...
        if (eq_pos) {
            ...
            *(value_end + 1) = '\0';
            ...
            else if (strcmp(key, "port") == 0) config_.broker_port = atoi(value);
